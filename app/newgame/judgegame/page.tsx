...
export default function JudgeGame() {
  ...
  const addJudge = async (judgeCode: string) => {
    return await fetch("api/addJudge", {
      method: "POST",
      credentials: "include",
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const code = formData.get("JudgeCode") as string
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(resData.message || "加入失败，请检查裁判码是否正确")
      }
    } catch (err) {
      setError("网络错误，请稍后重试")
    } finally {
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 via-blue-50/30 to-slate-50 dark:from-slate-950 dark:via-blue-950/30 dark:to-slate-950 p-4 relative overflow-hidden">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-1/4 -left-20 w-72 h-72 bg-blue-500/10 dark:bg-blue-400/5 rounded-full blur-3xl" />
        <div className="absolute bottom-1/4 -right-20 w-96 h-96 bg-purple-500/10 dark:bg-purple-400/5 rounded-full blur-3xl" />
      </div>
        ...
        {!isAdded ? (
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl p-8 border border-slate-200 dark:border-slate-800 backdrop-blur-sm relative overflow-hidden">
            ...
            <div className="text-center mb-8">
              ...
              <h1 className="text-3xl font-bold text-slate-900 dark:text-slate-100 mb-2 text-balance">裁判加入</h1>
              <p className="text-slate-600 dark:text-slate-400 text-pretty">请输入您的裁判码以加入比赛</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-2">
                ...
                <input
                  ...
                  type="text"
                  placeholder="输入裁判码"
                  className="w-full text-lg h-12 px-4 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-600 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 disabled:opacity-50 disabled:cursor-not-allowed"
                  disabled={isLoading}
                  required
                />
              </div>
              ...
              <button
                type="submit"
                className="w-full h-12 text-base font-medium rounded-lg bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 text-white transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
                disabled={isLoading}
              >
                {isLoading ? (
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl p-8 border border-slate-200 dark:border-slate-800 backdrop-blur-sm text-center animate-in fade-in zoom-in duration-500">
            ...
            <div className="bg-slate-100 dark:bg-slate-800/50 rounded-lg p-4 mb-6">
              <p className="text-sm text-slate-600 dark:text-slate-400 mb-1">裁判码</p>
              <p className="text-lg font-mono font-semibold text-slate-900 dark:text-slate-100">{judgeCode}</p>
            </div>
